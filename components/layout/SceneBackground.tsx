...
const SceneBackground = memo(function SceneBackground({
  ...
  useEffect(() => {
    ...
    /**
     * Resolve a color string to "r,g,b" format.
     * Handles:
    ...
    const resolveToRgb = (color: string): string => {
      try {
        // If it's a CSS variable, resolve via computed style
    ...
    const rgbP = resolveToRgb(t.primary);
    const rgbS = resolveToRgb(t.secondary);
    const rgbA = resolveToRgb(t.accent);
